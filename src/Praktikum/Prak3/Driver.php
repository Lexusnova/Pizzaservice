...
class Driver extends Page
{
    ...
    protected function generateView():void
    {
        ...
        $this->generatePageHeader('Driver');
...
HTML;
        foreach($data as $order) {
            ...
            $orderPrice = htmlspecialchars("13,50€");
  ...
  <form action="./Driver.php" method="POST" id="form{$order[0]}">
    ...
    <input type="radio" name="lieferstatus" id="form{$orderid}fertig" value="3"{$this->isChecked($order[3], 3)}>
    <label for="form{$orderid}unterwegs">unterwegs</label>
    <input type="radio" name="lieferstatus" id="form{$orderid}unterwegs" value="4"{$this->isChecked($order[3], 4)}>
    <label for="form{$orderid}geliefert">geliefert</label>
    <input type="radio" name="lieferstatus" id="form{$orderid}geliefert" value="5"{$this->isChecked($order[3], 5)}>
    <button type="submit" name="action" value="{$orderid}" tabindex="1" accesskey="s">Submit (s)</button>
  </form>
